    ...
    def main_loop(self):
        ...
        while close_time > time.time():
            ...
            if const_value == 1:
                ...
                print(
                    "Attacking PLC1 with false data injection constant value: ",
                    toggle_status,
                )
                self.set(ACTUATOR1, toggle_status)
                self.send(
                    ACTUATOR1, toggle_status, PLC1_ADDR
                )  # this wont prolly do anything (Just to be safe)
                # time.sleep(HMI_PERIOD_SEC) # we avoid sleeping too much here
                time.sleep(0.2)
                logging.info(
                    "FDI Constant: Actuator status changed to: %s" % toggle_status
                )
                ...
                print(
                    "Attacking PLC1 with false data injection not value: ",
                    toggle_status,
                )
                self.set(ACTUATOR1, toggle_status)
                self.send(
                ...
                time.sleep(HMI_PERIOD_SEC)
                logging.info(
                    "FDI toggle: Actuator status changed to %s" % toggle_status
                )
